...
void tkmu_simple_ref( const TrackObj_tkmu& in, PropTrackObj_tkmu& out ){
    ...
    std::cout << " SOFTWARE : Eta calculation " << std::endl;
    if (tk_aeta < 1.1){
        std::cout << " SOFTWARE : -- barrel " << std::endl;
        etaProp = 1.1;
        deta    = tk_z/550./cosh(tk_aeta);
    }
    else {
        std::cout << " SOFTWARE : -- endcap " << std::endl;
        ...
        std::cout << " SOFTWARE :       delta = " << delta << std::endl;
    }
    std::cout << " SOFTWARE :       deta = " << deta << std::endl;
